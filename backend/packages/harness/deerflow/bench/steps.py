...
def apply_boundary(orders: list[dict], upto: date | None) -> list[dict]:
    ...
    return [
        o
        for o in orders
        if o.get("placed") and date.fromisoformat(o["placed"]) <= (upto or date.max)
    ]
...
def normalise_order(order: dict) -> dict:
    ...
    out = {
        ...
        "shipped_on": (order.get("shipped") or "").upper() or None,
        "amount": order["total"],
...
def summarise_statuses(orders: list[dict]) -> dict:
    ...
    return {
        str(k): f"{round(100 * v / len(known), 1)}%"
        for k, v in Counter(o["status"] for o in known).items()
    }
